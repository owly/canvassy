  ...
  require(['jquery', 'compiled/behaviors/elementToggler'], function($, elementToggler) {
    module('elementToggler', {
      teardown: function() {
        ...
        _ref = [this.$trigger, this.$otherTrigger, this.$target, this.$target1, this.$target2];
        _results = [];
    ...
    return test('toggles multiple elements separated by spaces', function() {
      this.$trigger = $('<input type="checkbox" class="element_toggler" aria-controls="one two" />').appendTo('body');
      this.$target1 = $('<div id="one" style="display: none;">one</div>').appendTo('body');
      this.$target2 = $('<div id="two" style="display: none;">two</div>').appendTo('body');
      this.$trigger.prop('checked', true).trigger('change');
      ok(this.$target1.is(':visible'), 'first target is shown');
      return ok(this.$target2.is(':visible'), 'second target is shown');
    });
  });
